...
const Tabs = class {
    ...
    handleKeyDownIndicator = (e, direction) => {
        if (e.key === 'Enter') {
            direction === directions_types.INDICATOR_DIRECTIONS.left
                ? this.handleScroll(directions_types.INDICATOR_DIRECTIONS.left)
                : this.handleScroll(directions_types.INDICATOR_DIRECTIONS.right);
        }
    };
    ...
    findNextTabItemId(direction) {
        if (direction === directions_types.INDICATOR_DIRECTIONS.left) {
    ...
    renderSlots(type) {
        return this.tabElements.map((item, index$1) => {
            const slotname = (item.slot && item.slot.replace('tab-', '')) || `${index$1 + 1}`;
            ...
            return type === 'tabpanel' ? (index.h("div", { key: uuid, id: `tabpanel-${index$1 + 1}`, role: "tabpanel", "aria-labelledby": `tab-${index$1 + 1}`, tabIndex: this.activeTabIndex === index$1 + 1 ? 0 : -1, class: { 'is-hidden': this.activeTabIndex !== index$1 + 1 } }, index.h("slot", { name: `tab-${slotname}` }))) : (index.h("button", { key: uuid, id: `tab-${index$1 + 1}`, type: "button", role: "tab", disabled: item.hasAttribute('disabled'), class: `tab ${this.activeTabIndex === index$1 + 1 ? 'is-active-tab' : ''}`, "aria-selected": `${this.activeTabIndex === index$1 + 1}`, "aria-controls": `tabpanel-${index$1 + 1}`, "aria-describedby": `tabpanel-${index$1 + 1}`, onClick: () => this.handleClickOnButton(index$1 + 1), onKeyDown: this.handleKeyDown }, index.h("div", { class: "tab-content" }, index.h("p", { class: "tab-title" }, " ", item.getAttribute('data-title')), item.getAttribute('data-subtitle'))));
        });
    }
    ...
    render() {
        return (index.h(index.Host, { key: '037d9e35e1e160cdf67e79565edffe9ef6097ea7' }, index.h("div", { key: '3d9c826186d92b384cefb90a933f435fc991a1e3', class: `tabs tabs-${this.appearance}` }, index.h("div", { key: '91b6c377f1cc6b1040285e461c1aac15602c5a95', class: `tab-header ${this.isScrollable ? 'is-scrollable' : ''}` }, index.h("div", { key: 'b699dcdec27e1c7177272613505d5e5623a82c5a', class: "tab-list", role: "tablist", "aria-labelledby": "tablist-1", ref: (el) => (this.tabList = el) }, this.renderSlots('tab')), this.isScrollable && this.activeTabIndex > 1 && (index.h("wink-indicator", { class: "scroll-button scroll-button-left", direction: "left", tabIndex: 1, onKeyDown: (event) => this.handleKeyDownIndicator(event, directions_types.INDICATOR_DIRECTIONS.left), onClick: () => this.handleScroll(directions_types.INDICATOR_DIRECTIONS.left) })), this.isScrollable && this.activeTabIndex !== this.tabElements.length && (index.h("wink-indicator", { class: "scroll-button scroll-button-right", direction: "right", tabIndex: 1, onKeyDown: (event) => this.handleKeyDownIndicator(event, directions_types.INDICATOR_DIRECTIONS.right), onClick: () => this.handleScroll(directions_types.INDICATOR_DIRECTIONS.right) }))), this.renderSlots('tabpanel'))));
    }
};
Tabs.style = {
...
exports.wink_tabs = Tabs;

//# sourceMappingURL=wink-tabs.cjs.entry.js.map