...
const RangeSlider = class {
    ...
    renderSpecific() {
        ...
        return (index.h("div", { class: "slider-specific" }, this.sliderType === SLIDER_TYPES.range && (index.h("wink-input", { class: "slider-specific-from", type: "number", inputId: this.sliderId + 'from-value', name: "fromValue", label: "From", value: this.bottomValue.toString(), "min-value": this.minValue, "max-value": this.maxValue, step: this.step, isRequired: true, ...fromValueProps, onValueChanged: (event) => this.handleValueChanged(event, SLIDER_DIRECTIONS.from) })), index.h("wink-input", { class: "slider-specific-to", type: "number", inputId: this.sliderId + 'to-value', name: "toValue", label: this.sliderType === SLIDER_TYPES.range ? 'To' : '', value: this.value.toString(), "min-value": this.minValue, "max-value": this.maxValue, step: this.step, isRequired: true, ...toValueProps, onValueChanged: (event) => this.handleValueChanged(event, SLIDER_DIRECTIONS.to) })));
    }
    checkForErrors() {
    ...
    render() {
        return (index.h("label", { key: 'a9f898767e2d6e1e94cac11d09c9c78b24f010da', class: `slider-container slider-container-${this.sliderType}` }, index.h("wink-form-label", { key: '5652e8d44e355e459d5e1cd5646e72d1eb5e7322', label: this.label, helper: this.helper }), index.h("div", { key: '5c9b646830bb0774de70cf4a7e32f511731f71a0', class: "slider-wrapper" }, this.showValues && index.h("span", null, this.minValue), index.h("div", { key: 'e7ee2f3cd7896e4d3d0461e3806e7aea9c6a66e3', class: "slider-controls" }, index.h("div", { key: '388b3f0a1530ce5731556f136da3607828d126e4', class: `slider-tooltip-wrapper ${!this.showInput ? 'slider-tooltip-shown' : ''}` }, this.sliderType === SLIDER_TYPES.range && (index.h("div", { style: { left: this.calculateTooltipPosition(this.bottomValue) }, class: "slider-tooltip" }, this.bottomValue)), index.h("div", { key: '06a0ad3ccee21c8be0504fdd1688906764347de9', style: { left: this.calculateTooltipPosition(this.value) }, class: "slider-tooltip" }, this.value)), index.h("div", { key: 'b2acd782cecc66fb30a0278c5df7cf7b1fbb786e', style: { background: this.calculateBackground() }, class: "slider-track" }), this.sliderType === SLIDER_TYPES.range && (index.h("input", { class: "slider slider-from", type: "range", value: this.bottomValue, min: this.minValue, max: this.maxValue, step: this.step, onInput: (event) => this.handleValueSlider(event, SLIDER_DIRECTIONS.from) })), index.h("input", { key: 'bbc14c32e53527848d3ccaeebc97666976668853', class: "slider slider-to", type: "range", value: this.value, min: this.minValue, max: this.maxValue, step: this.step, onInput: (event) => this.handleValueSlider(event, SLIDER_DIRECTIONS.to) })), this.showValues && index.h("span", null, this.maxValue), this.sliderType === SLIDER_TYPES.single && this.showInput && this.renderSpecific()), this.sliderType === SLIDER_TYPES.range && this.showInput && this.renderSpecific(), this.topValueError && this.sliderType === SLIDER_TYPES.single && (index.h("wink-validation-message", { message: this.topValueError, type: "error" }))));
    }
};
RangeSlider.style = {
...
exports.wink_range_slider = RangeSlider;

//# sourceMappingURL=wink-range-slider.cjs.entry.js.map