...
export default {
  ...
  argTypes: {
    ...
    items: {
      description:
        'The items property gives the highlight links parameters to be displayed. Each link should have a text and a href property.',
      control: { type: 'array' },
  ...
  render: (args: any) => {
    return html`
      <wink-column-count cols-count="${args.colsCount}">
        ${args.items.map(
          (item: string) =>
            html` <wink-paragraph appearance="body" size="small" class="font--body-regular">${item} </wink-paragraph>`
        )}
      </wink-column-count>
    `;
  },
