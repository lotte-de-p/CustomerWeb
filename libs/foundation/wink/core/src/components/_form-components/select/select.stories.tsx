...
export default {
  ...
  parameters: {
    badges: [BADGES.ready],
    html: {
      transform: (code: string) => cleanScopedWinkElements(code, ['wink-select', 'option']),
    },
  },
  argTypes: {
    label: {
      control: 'text',
      description:
        'The label property specifies the label text displayed above the select, providing context or instructions for the user.',
      table: { category: STORY_CATEGORIES.cp },
    },
    helper: {
      control: 'text',
      description:
        'The helper property provides additional assistance or information related to the select in the form of helper text displayed below the label, offering guidance or clarification to the user.',
      table: { category: STORY_CATEGORIES.cp },
    },
    isRequired: {
      control: 'boolean',
    ...
    selectName: {
      control: 'text',
      description:
        'The selectName property assigns a name to the select, which is submitted with the form data when the form is submitted.',
      table: { category: STORY_CATEGORIES.cp },
    ...
    selectId: {
      control: 'text',
      description:
        'The selectId property assigns a unique identifier to the select, facilitating accessibility and functionality.',
      table: { category: STORY_CATEGORIES.cp },
    },
    selectItems: {
      control: 'object',
  ...
  render: (args) =>
    html` ${args.tooltip &&
      ...
      <wink-select
        ...
        ${args.selectItems.map(
          (item: string) => html`<option value="${item.toLowerCase().replace(' ', '-')}">${item}</option>`
        )}
      </wink-select>`,
} as Meta;
