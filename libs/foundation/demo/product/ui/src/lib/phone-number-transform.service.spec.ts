...
describe('PhoneNumberTransformService', () => {
  ...
  describe('autoFormatPhoneNumber', () => {
    ...
    it('should not convert an empty msisdn', () => {
      const actual = phoneNumberTransformService.autoFormatPhoneNumber('');
      expect(actual).toBe('');
    });

    it('should not convert an invalid msisdn', () => {
      const actual = phoneNumberTransformService.autoFormatPhoneNumber('+9177190945324456');
      expect(actual).toBe('0+9177190945324456');
    });
  });
});
