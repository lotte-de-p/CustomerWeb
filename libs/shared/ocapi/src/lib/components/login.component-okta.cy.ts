...
const AUTHENTICATE_ENDPOINT = '**/login/authorization**/**';
const AUTHORIZE_ENDPOINT = '**oauth2/default/v1/authorize**/**';

const JSONP_CALLBACK =
  'https://secure.local.telenet.be/oauth2/default/v1/authorize?client_id=0oa538ny8p7XSzxnj0x7&response_type=code&redirect_uri=https://api.int.telenet.be/ocapi/login/callback/telenet_be&state=6a6fee06-278e-452c-910a-e1696609ce08&nonce=f373edb6-fb44-4d93-a119-0e0a93dd02f5&scope=openid%20profile%20licenses%20telenet.scopes%20offline_access&claims=%7B%22id_token%22:%7B%22http://telenet.be/claims/roles%22:null%7D%7D&code_challenge=eRox6PqLR_O0pLl0nHQiIFvumRZq5OerV41EWe26UwI&code_challenge_method=S256&prompt=none';
...
describe('Login', () => {
  ...
  describe('Login Service', () => {
    it('should go to authorization when clicking the login button', () => {
      cy.window().then((win) => {
        cy.stub(win, 'open')
          .as('windowOpen')
          .callsFake(() => {
      ...
      cy.get('@windowOpen').should('have.been.calledWithMatch', (url: string) => {
        expect(url).to.include('ocapi/login/authorization/telenet_be');
        return true;
      });
    });
  });
  ...
  describe('Transparent Authentication', () => {
    it('should attempt to authenticate if getUserDetails fails and display content if it succeeds', () => {
      ...
      cy.intercept('GET', AUTHENTICATE_ENDPOINT, {
        statusCode: 302,
        headers: {
          location: JSONP_CALLBACK,
        },
      }).as(ATTEMPT_AUTHENTICATE);

      cy.intercept('GET', AUTHORIZE_ENDPOINT, {
        statusCode: 302,
      }).as(CALLBACK);
