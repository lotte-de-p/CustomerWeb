import userDetails from '../../../fixtures/fleet-user-product-overview/open-id/userdetails.json';
import products from '../../../fixtures/fleet-user-product-overview/products/products.json';
import identity from '../../../fixtures/fleet-user-product-overview/identities/identity-19012995.json';
...
describe('Fleet User Product Overview', () => {
  beforeEach(() => {
    ...
    cy.visit('/fleet-user-product-overview');
  });
...
function checkIfComponentsExists(): void {
  cy.get('tg-fleet-user-product-overview').should('exist');
...
function checkSpecificationValue(): void {
  cy.getElement('specificationValue').eq(0).should('contain.text', 'ng.fleet-user-product-overview.lbl.unlimited');
}
...
function checkLabelForSuspendedProduct(): void {
  cy.getElement('productSuspended').eq(0).should('contain.text', 'ng.fleet-user-product-overview.lbl.status-suspended');
}

function checkSuspendedPlanNotification(): void {
  cy.getElement('suspendedPlanNotification')
    .eq(0)
    .should('contain.text', 'ng.fleet-user-product-overview.lbl.suspended-plan');
}
