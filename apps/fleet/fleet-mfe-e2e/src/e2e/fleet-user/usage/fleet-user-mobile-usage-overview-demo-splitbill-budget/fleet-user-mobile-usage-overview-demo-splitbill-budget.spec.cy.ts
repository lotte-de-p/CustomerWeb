...
import usages from '../../../../fixtures/usage/fleet-user-mobile-usage-overview/mobile-service/sb/015373104/usages.json';
...
describe('Fleet User Mobile Usage Overview', () => {
  beforeEach(() => {
    ...
    cy.visit('/fleet-user-mobile-usage-overview-demo-splitbill-budget?includeMobileLineSelector=true');
  });

  it('On page load, form should be pre-populated with values', (): void => {
    checkIfComponentsExists();
    checkSplitbillBudget();
...
function checkSplitbillBudget(): void {
  cy.getElement('card-config-name')
    .eq(0)
    .should('contain.text', 'ng.fleet-user-mobile-usage-overview.lbl.split-bill-consumption');
}

function checkCardConfigUsedAmount(): void {
  cy.getElement('config-used-amount')
    .eq(0)
    .should('contain.text', 'fleet-user-mobile-usage-overview.lbl.monetary-balance');
}
...
function usedAndStartAmount(): void {
  cy.getElement('splitbill-budget-title')
    .eq(0)
    .should('contain.text', 'ng.fleet-user-mobile-usage-overview.lbl.split-bill-consumption');
  cy.getElement('splitbill-budget-value').eq(0).should('contain.text', '8,01  / 10,00 EUR');
}
