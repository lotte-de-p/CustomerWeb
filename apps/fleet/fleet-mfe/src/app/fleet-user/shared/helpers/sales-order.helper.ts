...
export class SalesOrderHelper {
  ...
  static getItemsByCategory(orderItems: OrderItem[], categoryList: string[]): OrderItem[] {
    return orderItems.filter((orderItem) => {
      const categories = orderItem && orderItem.productInfo.categories;
      return (
        categoryList &&
        categoryList.some((category) => {
          return categories && categories.includes(category.toLowerCase());
        })
      );
    });
  }
