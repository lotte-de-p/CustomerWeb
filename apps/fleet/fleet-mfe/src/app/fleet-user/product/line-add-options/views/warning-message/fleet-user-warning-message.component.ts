...
export class FleetUserWarningMessageComponent extends AbstractBaseComponent implements OnInit {
  ...
  @Output() closeWarning: EventEmitter<void> = new EventEmitter<void>();
          ...
          next: (response) => {
            if (response && response.success) {
              this.productDatalayerService.addEventToUDL(
                LineAddOptionsConstants.EVENT_NAME_LINE_DETAILS_CHANGED,
  ...
  private getAddOptionsPostData(): ManageModifyProductsInterface[] {
    if (this.selectedLine.isAffected && this.selectedLine.options) {
      return [this.createProductOptions(this.selectedLine)];
    }
  }
  ...
  private createProductOptions(selectedLine): ManageModifyProductsInterface {
    ...
    productOption['locationId'] = selectedLine.locationId;
