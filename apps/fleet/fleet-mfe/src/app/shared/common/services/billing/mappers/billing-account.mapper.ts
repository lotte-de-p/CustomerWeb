...
import { BillingAccountInterface, BillingAddressInterface } from '../interfaces/billing-accounts.interface';
import {
  RawBillingAccountInterface,
  RawBillingAddressInterface,
  RawDispatchMethodInterface,
} from '../interfaces/raw-billing-accounts.interface';
...
export class BillingAccountMapper implements MapperInterface<RawBillingAccountInterface[], BillingAccountInterface[]> {
  ...
  private createBillingAccounts(rawBillingAccounts: RawBillingAccountInterface[]): BillingAccountInterface[] {
    ...
    rawBillingAccounts.forEach((rawAccount) =>
      billingAccounts.push(this.createBillingAccount(rawAccount) as BillingAccountInterface)
    );
    return billingAccounts;
  }

  private createBillingAccount(rawBillingAccount: RawBillingAccountInterface): BillingAccountInterface | undefined {
  ...
  private updateDispatchMethod(dispatchMethod: RawDispatchMethodInterface) {
    if (FleetBillingConstants.DISPATCH_METHOD_PAPER.includes(dispatchMethod?.current)) {
