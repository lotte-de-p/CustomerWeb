...
export const handleCustomEvents = (StoryFn: () => unknown) => {
  ...
  useEffect(() => {
    ...
    const winkIcons = document.getElementById('winkIcons');
    const brandWinkCss = `https://customer-web-foundation-wink-cdn.aem-modules-dev.awscloud.external.telenet.be/v2.0.0/assets/shared/tokens/${brand}-light-theme.css`;
    const brandWinkIconsCss = `https://customer-web-foundation-wink-cdn.aem-modules-dev.awscloud.external.telenet.be/v2.0.0/assets/${brand}/icons/icons-font/${brand}-icons-font.css`;
    if (theme) {
    ...
    winkIcons?.setAttribute('href', brandWinkIconsCss);
