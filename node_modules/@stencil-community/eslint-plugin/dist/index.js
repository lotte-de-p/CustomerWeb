...
const SyntaxKind = ts__default["default"].SyntaxKind;
function isPrivate(originalNode) {
    const modifiers = ts__default["default"].canHaveModifiers(originalNode)
        ? ts__default["default"].getModifiers(originalNode)
        : undefined;
    if (modifiers) {
        return modifiers.some((m) => m.kind === ts__default["default"].SyntaxKind.PrivateKeyword ||
            m.kind === ts__default["default"].SyntaxKind.ProtectedKeyword);
    }
...
function getDecoratorList(originalNode) {
    const decorators = ts__default["default"].canHaveDecorators(originalNode)
        ? ts__default["default"].getDecorators(originalNode)
        : undefined;
    return decorators;
}
...
const rule$j = {
    ...
    create(context) {
        ...
        return Object.assign(Object.assign({}, stencil.rules), { 'PropertyDefinition': getStyle, 'MethodDefinition[kind=method]': getStyle });
    }
};
...
const rule$i = {
    ...
    create(context) {
        ...
        return Object.assign(Object.assign({}, stencil.rules), { 'PropertyDefinition > Decorator[expression.callee.name=Element]': (node) => {
                if (stencil.isComponent()) {
    ...
    create(context) {
        ...
        return Object.assign(Object.assign({}, stencil.rules), { 'PropertyDefinition': (node) => {
                ...
                if (stencil.isComponent() && propDecorator) {
                    ...
                    const originalNode = parserServices.esTreeNodeToTSNodeMap.get(node);
                    const hasReadonly = !!(ts__default["default"].canHaveModifiers(originalNode) &&
                        ((_a = ts__default["default"].getModifiers(originalNode)) === null || _a === void 0 ? void 0 : _a.some(m => m.kind === ts__default["default"].SyntaxKind.ReadonlyKeyword)));
                    if (!hasReadonly) {
                        context.report({
                            node: node.key,
...
const rule$6 = {
    ...
    create(context) {
        ...
        return Object.assign(Object.assign({}, stencil.rules), { 'PropertyDefinition > Decorator[expression.callee.name=Prop]': checkName, 'MethodDefinition[kind=method] > Decorator[expression.callee.name=Method]': checkName });
    }
};
            ...
            'ClassDeclaration': (node) => {
                ...
                if (component) {
                    ...
                    const nonTypeExports = typeChecker.getExportsOfModule(typeChecker.getSymbolAtLocation(originalNode.getSourceFile()))
                        .filter(symbol => (symbol.flags & (ts__default["default"].SymbolFlags.Interface | ts__default["default"].SymbolFlags.TypeAlias)) === 0)
